...
Enumerates every op on the shared ``ops/`` layer (name, safety risk, derived mutation bit,
and one-line summary) — "one operation, three projections" made introspectable, and the source the CLI
...
def register_operations_routes(app) -> None:
    ...
    async def _api_operations():
        ...
        specs = load_all()
        return {"operations": [s.as_dict() for s in sorted(specs.values(), key=lambda s: s.name)]}
